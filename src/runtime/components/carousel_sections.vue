            ...
            <div class="flex flex-col items-stretch gap-0">
                <div :style="{ '--c': e.color }" v-for="(e, i) in sections" @click="(ev) => next(i, ev.target)" class="carousel" :class="{ 'opacity-50': i !== selected }">
                    ...
                    <motion.div
                        ...
                        :initial="{ scaleX: 0 }"
                        :animate="{ scaleX: 1, transition: { duration: e.duration / 1000, type: 'tween', ease: 'linear' } }"
                    ></motion.div>
...
function next(index?: number, el?: EventTarget | undefined | null) {
    if (index !== undefined && index === selected.value) return;
...
.indicator {
    @apply absolute bottom-0 h-[2px] w-full rounded-full origin-left duration-1000 bg-[var(--c)];
}

.carousel {
    @apply relative collapse border-t-0 border-b border-r-0 border-l-0 rounded-none duration-0 lg:duration-[250ms] py-2 select-none transition-all ease-out !important;
    .collapse-title {
        @apply font-bold px-0 origin-left transition-all flex items-center gap-2 duration-[250ms] ease-out !important;
    }
}

.points-list {
    @apply flex flex-col items-stretch gap-4 font-medium;
    li {
