...
const props = defineProps({
  ...
  clearBtn: { type: Boolean, required: false, default: false },
  ...
  searchParams: { type: null, required: false },
...
function clear() {
  model.value = void 0;
  emit("changed", void 0, -1);
  if (props.closeOnSelect) {
    open.value = false;
  }
  modalEl.value?.close();
}
function elementClicked(element, index) {
  model.value = element;
...
<style scoped>
.element-button{@apply btn bg-transparent hover:bg-base-200 shadow-none border-base-content/20}
</style>
